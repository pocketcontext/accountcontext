...
function settings(app) {
  ...
  const header = env("ACCOUNTCONTEXT_TRUSTED_PROXY_HEADER");
  if (header) {
  ...
  const limits = env("ACCOUNTCONTEXT_RATE_LIMITS");
  if (limits === "true") {
    ...
    console.log("deploy: ACCOUNTCONTEXT_RATE_LIMITS must be true or false; rate limit settings left alone");
  }
}

function googleOAuth(app) {
  const clientId = String($os.getenv("ACCOUNTCONTEXT_GOOGLE_CLIENT_ID") || "");
  const clientSecret = String($os.getenv("ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET") || "");
  if (!clientId && !clientSecret) return;
  if (!clientId.trim() || !clientSecret.trim()) {
    throw new Error("ACCOUNTCONTEXT_GOOGLE_CLIENT_ID and ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET must be set together");
  }
  if (/\s/.test(clientId) || /\s/.test(clientSecret)) {
    throw new Error("AccountContext Google OAuth credentials must not contain whitespace");
  }
  try {
    // PocketBase's system migrations already created users before bootstrap
    // completes. AccountContext migrations deliberately preserve its OAuth options.
    ...
    throw new Error("Could not apply AccountContext Google OAuth configuration; server startup stopped");
  }
}
