...
IMAGE = "ghcr.io/pocketcontext/accountcontext:latest"
...
def registry_auth(credentials):
    ...
    old_env = ENV.copy()
    try:
        with tempfile.TemporaryDirectory(prefix="accountcontext-registry-") as directory:
...
def deploy():
    ...
    if len(matches) != 1:
        raise RuntimeError("Expected exactly one AccountContext container; refusing an ambiguous update")
    container = matches[0]
    if container["Config"]["Image"] != IMAGE and not re.fullmatch(
            r"ghcr\.io/pocketcontext/accountcontext@sha256:[0-9a-f]{64}", container["Config"]["Image"]):
        raise RuntimeError("AccountContext image does not match the fixed deployment target")
    ...
    try:
        ...
        if state["Running"] or state["ExitCode"] != 0 or state.get("OOMKilled"):
            raise RuntimeError("AccountContext did not stop gracefully; refusing to replace it")
        ...
        try:
            ...
            print("AccountContext recovery failed; operator intervention required", file=sys.stderr)
        raise
...
def main():
    ...
    with open("/run/lock/deploy-accountcontext.lock", "a") as lock:
