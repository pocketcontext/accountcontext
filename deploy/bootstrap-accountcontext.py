...
HOST = 'accounts.pocketcontext.com'
PAYLOAD = Path('/etc/accountcontext-bootstrap.json')


def validate(payload, credential):
    if not re.fullmatch(r'ghcr\.io/pocketcontext/accountcontext@sha256:[a-f0-9]{64}', payload.get('image', '')):
        raise ValueError('Invalid pinned application image')
    settings = payload.get('settings')
    allowed = {'ACCOUNTCONTEXT_' + key for key in ('SUPERUSER_EMAIL', 'SUPERUSER_PASSWORD', 'GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'GOOGLE_WORKSPACE_DOMAIN', 'TRUSTED_PROXY_HEADER')}
    ...
    if settings['LITESTREAM_BUCKET'] != 'accountcontext-backup' or settings['LITESTREAM_PATH'] != 'once-pocketcontext/accountcontext':
...
def main():
    ...
    with open('/run/lock/deploy-accountcontext.lock', 'w') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX)
        with tempfile.TemporaryDirectory(prefix='accountcontext-registry-') as temp:
            os.chmod(temp, 0o700)
            ...
            run(args, env)
            PAYLOAD.unlink()
    print('AccountContext initial deployment completed; private bootstrap payload removed.')
...
if __name__ == '__main__':
    try:
        ...
        print('AccountContext bootstrap failed (' + type(error).__name__ + '); no details logged.', file=sys.stderr)
        sys.exit(1)
