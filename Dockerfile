# AccountContext image for Basecamp ONCE: HTTP on port 80, GET /up, all state under /storage.
...
COPY docker/backup.py /usr/local/bin/accountcontext-backup.py
WORKDIR /app
...
ENV ACCOUNTCONTEXT_RATE_LIMITS=true
VOLUME /storage
EXPOSE 80
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/bin/entrypoint.sh"]

ARG REVISION=unknown
LABEL org.opencontainers.image.title="AccountContext" \
      org.opencontainers.image.description="Expenses operated through a coding agent with protected evidence and complete backups" \
      org.opencontainers.image.source="https://github.com/pocketcontext/accountcontext" \
