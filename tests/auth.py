...
def main():
    ...
    with patch.dict(os.environ, {'ACCOUNTCONTEXT_GOOGLE_CLIENT_ID': 'synthetic-client',
                                 'ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET': 'synthetic-secret'}), server(args.binary) as request:
        ...
        project = request('POST', '/api/collections/parties/records', {'name': 'Access test'}, token)
        project_path = '/api/collections/parties/records/' + project['id']
        ...
        def rejected(old_token):
            for method, endpoint, body in [
                ...
                ('POST', '/api/context/query', {'sql': 'SELECT * FROM parties'}),
                ('GET', '/api/collections/parties/records', None),
                ('GET', project_path, None),
                ('POST', '/api/collections/parties/records', {'name': 'Forbidden'}),
                ...
                ('POST', '/api/batch', {'requests': [
                    {'method': 'POST', 'url': '/api/collections/parties/records', 'body': {'name': 'Forbidden'}},
                ]}),
            ]:
        ...
        assert request('POST', '/api/context/query', {'sql': 'SELECT created_by FROM parties'}, fresh)['rows'] == [[user['id']]]
        assert request('GET', '/api/collections/parties/records', token=admin)['totalItems'] == 1
