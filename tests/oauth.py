...
spec = importlib.util.spec_from_file_location('tc', Path(__file__).resolve().parents[1] / 'skills/accountcontext/scripts/ac.py')
...
class OAuthTest(unittest.TestCase):
    def setUp(self):
        ...
        self.cfg = {'url': 'https://accounts.example.com', 'email': 'member@example.com', 'password': 'unused-password'}
    ...
    def test_oauth_configuration_needs_no_password(self):
        with patch.dict(os.environ, {'ACCOUNTCONTEXT_URL': self.cfg['url'], 'ACCOUNTCONTEXT_USER_EMAIL': self.cfg['email']}):
            os.environ.pop('ACCOUNTCONTEXT_USER_PASSWORD', None)
