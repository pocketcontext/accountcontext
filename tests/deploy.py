...
def main():
    ...
    env = smoke.once_env({'ACCOUNTCONTEXT_RATE_LIMITS': 'true',
                          'ACCOUNTCONTEXT_TRUSTED_PROXY_HEADER': 'X-Forwarded-For'})
    with patch.dict(os.environ, env), server(binary) as request:
        ...
        assert collection['oauth2']['providers'][0]['clientId'] == env['ACCOUNTCONTEXT_GOOGLE_CLIENT_ID']
        assert env['ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET'] not in json.dumps(collection)
    ...
    for values in [
        {'ACCOUNTCONTEXT_GOOGLE_CLIENT_ID': 'synthetic-client'},
        {'ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET': 'synthetic-secret'},
        {'ACCOUNTCONTEXT_GOOGLE_CLIENT_ID': 'has whitespace', 'ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET': 'synthetic-secret'},
    ]:
        with tempfile.TemporaryDirectory(prefix='accountcontext-invalid-config-') as tmp:
            clean_env = {k: v for k, v in os.environ.items() if not k.startswith('ACCOUNTCONTEXT_')}
