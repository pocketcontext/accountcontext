...
class BackupTests(unittest.TestCase):
    def setUp(self):
        ...
        self.original.write_bytes(b'Synthetic original invoice USD 129.00\n')
        self.conn = sqlite3.connect(self.data / 'data.db')
        self.addCleanup(self.conn.close)
        self.conn.executescript('PRAGMA journal_mode=WAL; CREATE TABLE _collections(id TEXT,name TEXT); INSERT INTO _collections VALUES("col","documents"); CREATE TABLE documents(id TEXT,original TEXT,sha256 TEXT);')
        self.conn.execute('INSERT INTO documents VALUES(?,?,?)', ('doc','synthetic.txt',backup.digest(self.original)))
        self.conn.commit()
    ...
    def test_newer_live_reference_not_in_snapshot_is_detected(self):
        ...
        self.conn.execute('INSERT INTO documents VALUES(?,?,?)',('later','absent.txt','a'*64));self.conn.commit()
        with self.assertRaises(RuntimeError): backup.verify(self.data)
