...
hook = load("hook", "deploy-accountcontext.py")
...
class DeploymentTests(unittest.TestCase):
    ...
    def test_adopts_initial_pinned_accountcontext_image(self):
        calls = self.run_hook(image="ghcr.io/pocketcontext/accountcontext@sha256:" + "a" * 64)
        self.assertEqual(calls[-1], ("once", "update", hook.HOST, "--image", hook.IMAGE, "--auto-update=false"))
    ...
    def test_arguments_rejected_before_commands(self):
        with patch.object(sys, "argv", ["deploy-accountcontext", "other-host"]), patch.object(hook, "run") as run:
            with self.assertRaises(RuntimeError):
                hook.main()
