...
def main():
    ...
    with patch.dict(os.environ, {'ACCOUNTCONTEXT_GOOGLE_WORKSPACE_DOMAIN': 'example.com'}), google_fixture() as (url, codes), server(args.binary) as request:
...
def no_domain_checks(binary):
    with patch.dict(os.environ, {'ACCOUNTCONTEXT_GOOGLE_WORKSPACE_DOMAIN': ''}), google_fixture() as (url, codes), server(binary) as request:
        admin = request('POST', '/api/collections/_superusers/auth-with-password', {
            'identity': 'admin@example.com', 'password': 'SyntheticAdminPassword123!',
        })['token']
        provider = {'name': 'google', 'clientId': 'synthetic-client', 'clientSecret': 'synthetic-secret',
