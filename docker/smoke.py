#!/usr/bin/env python3
"""Checks of the AccountContext container image. Python 3 standard library and the docker CLI only.
...
def superuser_token(base, email, password):
    ...
    check(True, 'superuser login with ACCOUNTCONTEXT_SUPERUSER_EMAIL and ACCOUNTCONTEXT_SUPERUSER_PASSWORD')
...
EVIDENCE = b'Synthetic original invoice: USD 129.00. No real financial data.\n'


def write_record(client, user_id):
    boundary = 'accountcontext-' + secrets.token_hex(12)
    body = bytearray()
    for name, value in {'title': 'Synthetic image invoice', 'owner': user_id}.items():
        body.extend(f'--{boundary}\r\nContent-Disposition: form-data; name="{name}"\r\n\r\n{value}\r\n'.encode())
    body.extend(f'--{boundary}\r\nContent-Disposition: form-data; name="original"; filename="invoice.txt"\r\nContent-Type: text/plain\r\n\r\n'.encode())
    body.extend(EVIDENCE)
    body.extend(f'\r\n--{boundary}--\r\n'.encode())
    request = urllib.request.Request(client.base + '/api/collections/documents/records', data=bytes(body),
...
def check_records(client, document, metadata):
    rows = client.sql(f"SELECT title FROM documents WHERE id = '{document}'")
    check(rows == [['Synthetic image invoice']], 'synthetic document survives persistence and restore')
...
def once_env(extra=None):
    ...
    env = {
        'BASE_URL': 'https://accounts.example.test', 'SECRET_KEY_BASE': secret(secrets.token_hex(32)), 'DISABLE_SSL': 'true', 'NUM_CPUS': '2',
        ...
        'ACCOUNTCONTEXT_SUPERUSER_EMAIL': 'operator@example.test', 'ACCOUNTCONTEXT_SUPERUSER_PASSWORD': secret('-' + secrets.token_urlsafe(24)),
        'ACCOUNTCONTEXT_GOOGLE_CLIENT_ID': 'image-test.apps.googleusercontent.com',
        'ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET': secret(secrets.token_urlsafe(24)),
        'ACCOUNTCONTEXT_GOOGLE_WORKSPACE_DOMAIN': 'example.test',
    }
    env.update(extra or {})
    return env
...
def smoke(image, tmp, run_id):
    name, volume = f'ac-smoke-{run_id}', f'ac-smoke-{run_id}'
    ...
    token = superuser_token(base, env['ACCOUNTCONTEXT_SUPERUSER_EMAIL'], env['ACCOUNTCONTEXT_SUPERUSER_PASSWORD'])
    ...
    check(settings['rateLimits']['enabled'] is True, "rate limits are enabled by the image's default ACCOUNTCONTEXT_RATE_LIMITS=true")
    ...
    check(collection['oauth2']['providers'][0]['clientId'] == env['ACCOUNTCONTEXT_GOOGLE_CLIENT_ID'], 'Google client ID matches the environment')
    check(env['ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET'] not in json.dumps(collection), 'the collection API does not return the Google secret')
...
def config(image, tmp, run_id):
    ...
    expect_startup_error(image, 'a superuser email without a password', {'LITESTREAM_DISABLED': 'true', 'ACCOUNTCONTEXT_SUPERUSER_EMAIL': 'operator@example.test'},
                         ['ACCOUNTCONTEXT_SUPERUSER_PASSWORD'])

    expect_startup_error(image, 'a Google client ID without a secret',
                         {'LITESTREAM_DISABLED': 'true', 'ACCOUNTCONTEXT_GOOGLE_CLIENT_ID': 'image-test.apps.googleusercontent.com'},
                         ['ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET'])
    expect_startup_error(image, 'a Google secret without a client ID',
                         {'LITESTREAM_DISABLED': 'true', 'ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET': secret(secrets.token_urlsafe(24))},
                         ['ACCOUNTCONTEXT_GOOGLE_CLIENT_ID'])
    ...
    run_app(image, name, f'ac-config-{run_id}', env)
    time.sleep(20)
    running, code = state(name)
    text = check_logs(name)
...
def restore(image, tmp, run_id):
    network, minio, bucket = f'ac-drill-{run_id}', f'ac-drill-minio-{run_id}', 'accountcontext-drill'
    ...
    def app_env(sync_interval):
        return once_env({'LITESTREAM_BUCKET': bucket, 'LITESTREAM_PATH': f'drill-{run_id}/data', 'LITESTREAM_REGION': 'us-east-1',
                         ...
                         'LITESTREAM_SECRET_ACCESS_KEY': minio_password, 'LITESTREAM_SYNC_INTERVAL': sync_interval, 'ACCOUNTCONTEXT_BACKUP_INTERVAL': '5'})
    ...
    first, second, third = (f'ac-drill-{letter}-{run_id}' for letter in 'abc')
    ...
    superuser_token(base, env['ACCOUNTCONTEXT_SUPERUSER_EMAIL'], env['ACCOUNTCONTEXT_SUPERUSER_PASSWORD'])
...
def main():
    ...
    failed = True
    with tempfile.TemporaryDirectory(prefix='accountcontext-image-') as tmp:
        try:
