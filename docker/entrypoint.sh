...
serve() {
	...
	unset LITESTREAM_ACCESS_KEY_ID LITESTREAM_SECRET_ACCESS_KEY \
		AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY ACCOUNTCONTEXT_SUPERUSER_PASSWORD
...
if [ -n "${ACCOUNTCONTEXT_GOOGLE_CLIENT_ID:-}" ] && [ -z "${ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET:-}" ]; then
	die "ACCOUNTCONTEXT_GOOGLE_CLIENT_ID requires ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET"
elif [ -n "${ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET:-}" ] && [ -z "${ACCOUNTCONTEXT_GOOGLE_CLIENT_ID:-}" ]; then
	die "ACCOUNTCONTEXT_GOOGLE_CLIENT_SECRET requires ACCOUNTCONTEXT_GOOGLE_CLIENT_ID"
fi
...
if [ "$replicate" = true ]; then
	python3 /usr/local/bin/accountcontext-backup.py restore || die "complete evidence restore failed"
	if [ -f "$DB_PATH" ]; then
...
python3 /usr/local/bin/accountcontext-backup.py verify || die "evidence verification failed"

if [ -n "${ACCOUNTCONTEXT_SUPERUSER_EMAIL:-}" ] && [ -n "${ACCOUNTCONTEXT_SUPERUSER_PASSWORD:-}" ]; then
	log "upserting the superuser from ACCOUNTCONTEXT_SUPERUSER_EMAIL"
	# shellcheck disable=SC2046 # see serve
	if ! "$SERVER" superuser upsert $(app_flags) -- "$ACCOUNTCONTEXT_SUPERUSER_EMAIL" "$ACCOUNTCONTEXT_SUPERUSER_PASSWORD"; then
		die "superuser upsert failed"
	fi
elif [ -n "${ACCOUNTCONTEXT_SUPERUSER_EMAIL:-}" ]; then
	die "ACCOUNTCONTEXT_SUPERUSER_EMAIL is set but ACCOUNTCONTEXT_SUPERUSER_PASSWORD is missing"
elif [ -n "${ACCOUNTCONTEXT_SUPERUSER_PASSWORD:-}" ]; then
	die "ACCOUNTCONTEXT_SUPERUSER_PASSWORD is set but ACCOUNTCONTEXT_SUPERUSER_EMAIL is missing"
fi

if [ "$replicate" = true ]; then
	log "starting Litestream, which starts and supervises the server"
	exec python3 /usr/local/bin/accountcontext-backup.py supervise litestream replicate -config "$LITESTREAM_CONFIG_FILE" -exec "$SELF serve"
fi
serve
