...
Online SQLite backup establishes the snapshot point. Documents are immutable and cannot
...
DATA = Path(os.environ.get('ACCOUNTCONTEXT_DATA_DIR', '/storage/pb_data'))
...
def refs(data):
    ...
    with closing(sqlite3.connect(f'file:{db}?mode=ro', uri=True)) as conn:
        ...
        if not conn.execute("SELECT 1 FROM sqlite_master WHERE name='documents' AND type='table'").fetchone():
            return {}
        collection = conn.execute("SELECT id FROM _collections WHERE name='documents'").fetchone()[0]
        result = {}
        for record, filename, sha in conn.execute('SELECT id, original, sha256 FROM documents'):
            if not filename:
                raise RuntimeError('document has no original')
            ...
            if len(sha) != 64 or any(c not in '0123456789abcdef' for c in sha):
                raise RuntimeError('document has no valid checksum')
...
def _upload(data):
    ...
    with tempfile.TemporaryDirectory(prefix='accountcontext-snapshot-') as temp:
        root = Path(temp)
...
def restore(data):
    ...
    with tempfile.TemporaryDirectory(prefix='accountcontext-restore-', dir=data.parent) as temp:
        root = Path(temp)
        archive = root / 'snapshot.tar.gz'
...
def supervise(command):
    ...
    interval = int(os.environ.get('ACCOUNTCONTEXT_BACKUP_INTERVAL', '3600'))
